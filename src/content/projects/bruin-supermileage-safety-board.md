...
**Limitations:** the INA and H₂ sensor were placed too far apart on the PCB, making the sensor output unreliable; the 0 Ω debug jumpers allowed us to reroute the connection without redesigning the PCB. Much of the AND-gate logic was verified through LTspice rather than systematic physical bench testing — given the timeline, we prioritized the most critical safety functions. We also estimated relay switching time from video rather than measuring it directly; a future revision would use an oscilloscope to measure switching time accurately.
